...
class ConfusionMatrix:
	...
	def recall(self, name=None):
		"""
		@rtype: float | Number[float, float]
		"""
		if name is None:
	...
	def entropy_freqs(self, ignore_class={""}):
		for i, name in enumerate(self._colnames):
			...
			for j, rname in zip(col, self._colnames):
				if j and rname not in ignore_class:
					p = j/size
					h_cluster -= j*log(p, 2.)
	...
	def entropy(self, ignore_class={""}):  # as in doi://10.1093/bioinformatics/btm134
		...
		# 	l, self._mat.shape[0], self._mat.shape[1], self._rowindex.keys()))
		totalsize = .0
		h = .0
	...
	def misclassification_rate(self, ignore_class={""}):
		...
		for iclass in ignore_class:
			try:
				ignorecolindex.add(self._colindex[iclass])
			except KeyError:
				pass
		totalreject = 0
		totalsize = 0
		...
		for name, row in zip(self._rownames, self._mat):
			if name not in ignore_class:
				...
				try:
					...
					pass
				for icolindex in ignorecolindex:
					totalreject += row[icolindex]
		if totalsize:
	...
	def plot_matrix(
		...
		reds = colors.LinearSegmentedColormap.from_list(
			"customreds", ["white", groupcols[0]], N=numcolors)  # pyplot.get_cmap( "Reds" )
		blues = colors.LinearSegmentedColormap.from_list(
			"customblues", ["white", groupcols[1]], N=numcolors)  # pyplot.get_cmap( "Blues" )
		greys = colors.LinearSegmentedColormap.from_list(
			"customgreys", ["white", groupcols[2]], N=numcolors)  # pyplot.get_cmap( "Greys" )
		...
		titlefont = font_manager.FontProperties(
			family="sans-serif", stretch="normal", weight="normal", size="medium", style="normal")
		extrafont = font_manager.FontProperties(
			family="serif", stretch="normal", weight="normal", size="small", style="italic")
